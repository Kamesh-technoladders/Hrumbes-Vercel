    ...
    const fetchTimelineData = async () => {
      try {
        ...
        const candidateEventsData = candidateTimelineData?.map((event) => {
          ...
          return {
            ...
            description: [
              ...
              candidate?.hr_jobs?.title ? `For ${candidate.hr_jobs.title}` : "",
            ].filter(Boolean),
            created_at: event.created_at || new Date().toISOString(),
            details: { candidate_name: candidate?.name, job_title: candidate?.hr_jobs?.title },
          };
        }) || [];
            ...
            const events: TimelineEvent[] = [
              {
                ...
                created_at: pe.created_at || new Date().toISOString(),
                details: {
        ...
        const { data: leaveData, error: leaveError } = await supabase
          .from("leave_requests")
          .select(
            "id, employee_id, leave_type_id, start_date, end_date, total_days, status, notes, created_at, approved_at, approved_by, cancelled_at, cancelled_by, hr_employees:hr_employees!leave_requests_employee_id_fkey(first_name, last_name), hr_employees_approved:hr_employees!leave_requests_approved_by_fkey(first_name, last_name), hr_employees_cancelled:hr_employees!leave_requests_cancelled_by_fkey(first_name, last_name)"
          )
          .order("created_at", { ascending: false });
        ...
        const leaveEventsData = leaveData?.flatMap((leave) => {
          ...
          if (leave.approved_at && leave.approved_by) {
            events.push({
              id: `${leave.id}-approved`,
              type: "leave",
              description: [
                `${leave.hr_employees?.first_name || "Unknown"} ${leave.hr_employees?.last_name || ""} leave approved`,
                `By ${leave.hr_employees_approved?.first_name || "Unknown"} ${leave.hr_employees_approved?.last_name || ""}`,
                `From ${leave.start_date} to ${leave.end_date} (${leave.total_days} days)${leave.notes ? ` (${leave.notes})` : ""}`,
              ],
              created_at: leave.approved_at,
              details: {
                employee_name: `${leave.hr_employees?.first_name || "Unknown"} ${leave.hr_employees?.last_name || ""}`,
                approver: `${leave.hr_employees_approved?.first_name || "Unknown"} ${leave.hr_employees_approved?.last_name || ""}`,
                status: "approved",
              },
            });
          }
          if (leave.cancelled_at && leave.cancelled_by) {
            events.push({
              id: `${leave.id}-cancelled`,
              type: "leave",
              description: [
                `${leave.hr_employees?.first_name || "Unknown"} ${leave.hr_employees?.last_name || ""} leave cancelled`,
                `By ${leave.hr_employees_cancelled?.first_name || "Unknown"} ${leave.hr_employees_cancelled?.last_name || ""}`,
                `From ${leave.start_date} to ${leave.end_date} (${leave.total_days} days)${leave.notes ? ` (${leave.notes})` : ""}`,
              ],
              created_at: leave.cancelled_at,
              details: {
                employee_name: `${leave.hr_employees?.first_name || "Unknown"} ${leave.hr_employees?.last_name || ""}`,
                canceller: `${leave.hr_employees_cancelled?.first_name || "Unknown"} ${leave.hr_employees_cancelled?.last_name || ""}`,
                status: "cancelled",
              },
            });
          }
        ...
        setLeaveEvents(leaveEventsData);

        // Fetch Time Logs Timeline
        const { data: timeLogData, error: timeLogError } = await supabase
          .from("time_logs")
          .select(
            "id, employee_id, date, clock_in_time, clock_out_time, status, is_submitted, is_approved, approved_at, approved_by, hr_employees:hr_employees!time_logs_employee_id_fkey(first_name, last_name), hr_employees_approved:hr_employees!time_logs_approved_by_fkey(first_name, last_name)"
          )
          .order("created_at", { ascending: false });
        ...
        const timeLogEventsData = timeLogData?.flatMap((log) => {
          const events: TimelineEvent[] = [
            {
              ...
              type: "time_log",
              description: [
                `${log.hr_employees?.first_name || "Unknown"} ${log.hr_employees?.last_name || ""} logged in`,
                `At ${log.clock_in_time ? new Date(log.clock_in_time).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" }) : "N/A"} on ${log.date}`,
              ],
              created_at: log.clock_in_time || log.created_at || new Date().toISOString(),
              details: {
          ...
          if (log.clock_out_time) {
            events.push({
              ...
              type: "time_log",
              description: [
                `${log.hr_employees?.first_name || "Unknown"} ${log.hr_employees?.last_name || ""} logged out`,
                `At ${new Date(log.clock_out_time).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })} on ${log.date}`,
              ],
              created_at: log.clock_out_time,
              details: {
                employee_name: `${log.hr_employees?.first_name || "Unknown"} ${log.hr_employees?.last_name || ""}`,
                date: log.date,
          ...
          if (log.approved_at && log.approved_by) {
            events.push({
              id: `${log.id}-approved`,
              type: "time_log",
              description: [
                `${log.hr_employees?.first_name || "Unknown"} ${log.hr_employees?.last_name || ""} time log approved`,
                `By ${log.hr_employees_approved?.first_name || "Unknown"} ${log.hr_employees_approved?.last_name || ""}`,
                `For ${log.date}`,
              ],
              created_at: log.approved_at,
              details: {
                employee_name: `${log.hr_employees?.first_name || "Unknown"} ${log.hr_employees?.last_name || ""}`,
                approver: `${log.hr_employees_approved?.first_name || "Unknown"} ${log.hr_employees_approved?.last_name || ""}`,
                date: log.date,
                status: "approved",
              },
            });
        ...
        setTimeLogEvents(timeLogEventsData);
      } catch (error: any) {
