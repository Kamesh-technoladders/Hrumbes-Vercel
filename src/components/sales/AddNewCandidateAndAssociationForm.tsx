...
// // Zod schema for the combined form data
...
//   // Fields for candidate_companies association
...
//         // Optional: Consider if you want to update existing candidate's phone/linkedin here?
//         // For now, we just use the existing ID.
//       } else {
...
//       // 2. Prepare data for candidate_companies insertion
//       // Ensure candidate_id matches the type in your DB (UUID or TEXT)
//       const associationData: CandidateCompanyInsert = {
//         // Cast to text if DB column is text, otherwise use UUID directly if DB column is uuid
//         candidate_id: candidateIdToUse, // Assuming candidate_id is UUID in candidate_companies now
//         company_id: companyId,
//         job_id: null, // From form - Ensure this logic is correct
//         designation: formData.designation || null,
//         contact_owner: formData.contact_owner || null,
//         contact_stage: formData.contact_stage || null,
//         created_by: (await supabase.auth.getUser()).data.user?.id, // Set creator
      
//       };

//       // 3. Insert into candidate_companies
//       console.log("Inserting into candidate_companies:", associationData);
//       const { error: associationError } = await supabase
//         .from('candidate_companies')
//         .insert(associationData);

//       if (associationError) {
//         console.error("Error inserting into candidate_companies:", associationError);
//         if (associationError.code === '23505') { // Unique constraint violation
//           throw new Error(`This candidate/job association already exists for this company.`);
//         }
//         // If candidate creation succeeded but association failed, the new candidate still exists.
//         // More complex logic could attempt to delete the candidate here, but usually not necessary.
...
//         ? `${result?.candidateName} already existed and was associated.`
...
//         <FormField control={form.control} name="email" render={({ field }) => (<FormItem><FormLabel>Email*</FormLabel><FormControl><Input type="email" placeholder="candidate@example.com (used to check duplicates)" {...field} /></FormControl><FormMessage /></FormItem>)} />
...
//         <h4 className="font-medium text-md border-b pb-1 pt-4">Association Details for this Company</h4>
//          <FormField control={form.control} name="job_id" render={({ field }) => (<FormItem><FormLabel>Job ID* (Role Reference)</FormLabel><FormControl><Input placeholder="Enter Job ID reference for this role" {...field} /></FormControl><FormMessage /></FormItem>)} />
//          <FormField control={form.control} name="designation" render={({ field }) => (<FormItem><FormLabel>Job Title (Designation)</FormLabel><FormControl><Input placeholder="e.g., Software Engineer" {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>)} />
//          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
