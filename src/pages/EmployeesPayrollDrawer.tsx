...
interface EmployeesPayrollDrawerProps {
  ...
  selectedEmployee: { id: string; employee_id: string; first_name: string; last_name: string; position?: string; joining_date?: string } | null;
}
...
const EmployeesPayrollDrawer = ({ isOpen, onOpenChange, selectedEmployee }: EmployeesPayrollDrawerProps) => {
  ...
  useEffect(() => {
    if (isOpen && selectedEmployee) {
      ...
      fetchLatestPaymentRecord();
    }
  }, [isOpen, selectedEmployee]);

  const fetchLatestPaymentRecord = async () => {
  ...
  const calculateTotals = (currentRecord: PaymentRecord, currentCustomDeductions: PaymentCustomDeduction[]) => {
    ...
    return {
      updatedRecord: {
        ...
        earnings: { ...earnings, total_earnings: totalEarnings, fixed_allowance: fixedAllowance },
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Step 1: Compare with previous payment to detect appraisal
      ...
      if (isUpdate) {
        ...
        // Step 2: Insert into appraisal_records if this is an appraisal
                ...
                {paymentRecord.deductions.lop_days > 0 && (
                  <div className="mt-4 pt-4 border-t">
                    <div className="flex justify-between items-center text-amber-800">
                      ...
                      <span className="font-mono font-medium">Calculated Internally</span>
                    </div>
